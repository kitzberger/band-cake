  ...
  init: function() {
    ...
    this.on("success", function(file, response) {
        if (null !== response) {
            ...
            let select = jQuery('<select id="file-song-select-' + response.file + '" style="height: 2em; padding: 2px;" />')
...
function vote(el, value) {
    ...
    if (related_to[0] == 'date') { data = {date_id:related_to[1],user_id:user_id,vote:value}; }
    if (related_to[0] == 'idea') { data = {idea_id:related_to[1],user_id:user_id,vote:value}; }
...
function init_calendar() {
    ...
    $('#calendar').fullCalendar({
        ...
        select: function(start, end) {
            ...
            if (title) {
                var event = prepareNewEvent(this)
        ...
        drop: function(date, dropEvent, dropItem, dropTarget) {
            ...
            var event = prepareNewEvent(this, date);
            var eventId = event.id;
...
function prepareNewEvent(cal, date) {
    ...
    if (typeof date === "object") {
        ...
        var eventTemplate = $(cal).data('event-template');
...
function init_comments() {
    var commentTable = $('table.comments');
    var togglePanel = commentTable.find('.toggle-old-ones');
    var showOldOnes = commentTable.find('.toggle-old-ones a.show-old-ones');
    var hideOldOnes = commentTable.find('.toggle-old-ones a.hide-old-ones');

    var oldComments = commentTable.find('tr.older-than-last-month');
    var newComments = commentTable.find('tr.younger-than-last-month');
